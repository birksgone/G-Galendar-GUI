...
def load_all_data(latest_folder, diff_folder, lang_folder):
    """Loads all necessary CSV files, including the hero master."""
    data = {}
    latest_dir = EVENT_BASE_DIR / latest_folder
    
    # Find and load the hero master CSV first
    try:
        ...
        raise FileNotFoundError(f"Hero master CSV not found in '{latest_dir}'. "
                                "Please generate it first if this is a new folder.")
...
if config.get("event_folder"):
    try:
        # --- Auto-generate Hero Master if not found ---
        latest_dir = EVENT_BASE_DIR / config["event_folder"]
        try:
            next(latest_dir.glob("*_private_heroes_*_en.csv"))
        except StopIteration:
            st.warning(f"Hero master not found for '{config['event_folder']}'. Generating automatically...")
            command = [
                sys.executable, # Use the same python that runs streamlit
                HERO_GEN_SCRIPT_PATH,
                "--version_folder", config["event_folder"],
                "--cutoff_date", "2300-01-01"
            ]
            with st.spinner(f"Running hero generator script... This may take a moment."):
                # ★★★ Corrected: subprocess call with error handling ★★★
                result = subprocess.run(command, capture_output=True, text=True, encoding='utf-8', errors='replace')

            if result.returncode != 0:
                st.error("Error during hero master generation:")
                st.code(result.stderr or result.stdout)
                st.stop() # Stop execution if generation fails
            else:
                st.success("Hero master generated successfully!")
        ...
        # (Rest of the UI and data processing...)
        ...
        if start_date_filter.isoformat() != config.get('filter_start_date'):
            config['filter_start_date'] = start_date_filter.isoformat(); config_changed = True
        if end_date_filter.isoformat() != config.get('filter_end_date'):
            config['filter_end_date'] = end_date_filter.isoformat(); config_changed = True
        if timezone != config.get('timezone'):
            config['timezone'] = timezone; config_changed = True
        if config_changed:
            save_json_file(CONFIG_FILE, config)
        ...
        start_dt = pd.to_datetime(start_date_filter); end_dt = pd.to_datetime(end_date_filter)
