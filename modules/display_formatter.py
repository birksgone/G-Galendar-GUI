...
def _check_condition(row, condition):
    ...
    if op == 'equals':
        return cell_value == str(val)
    if op == 'contains':
        return str(val) in cell_value
    if op == 'starts_with':
        return cell_value.startswith(str(val))
    if op == 'ends_with':
        return cell_value.endswith(str(val))
    if op == "matches":
...
def format_dataframe_for_display(df, type_mapping_rules, en_map, ja_map):
    df_copy = df.copy()
